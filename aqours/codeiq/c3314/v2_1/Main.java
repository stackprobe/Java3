...
public class Main {
	...
	private static Map<String, int[][]> _islandsCache = new TreeMap<String, int[][]>(StringTools.comp);
	private static Map<String, int[][]> _woodsCache = new TreeMap<String, int[][]>(StringTools.comp);
	...
	public static class SearchGWD {
		...
		private long action(int ground, int wood, int div) throws Exception {
			...
			// cache
			{
				String k = div + ":" + islandsTotalMax;
				arrIslands = _islandsCache.get(k);
				//System.out.println(k + " -> " + arrIslands); // test
				if(arrIslands == null) {
					arrIslands = new MaskIslands().get(div, islandsTotalMax);
					_islandsCache.put(k, arrIslands);
				}
			}

			for(int[] islands : arrIslands) {
				int islandsTotal = 0;
				...
				// cache
				{
					String k = String.join(":", StringTools.toStrings(islands)) + ":" + woodRem;
					arrWoods = _woodsCache.get(k);
					//System.out.println(k + " -> " + arrWoods); // test
					if(arrWoods == null) {
						arrWoods = new AllocWoods().get(islands, woodRem);
						_woodsCache.put(k, arrWoods);
					}
				}

				for(int[] woods : arrWoods) {
					int woodsTotal = 0;
